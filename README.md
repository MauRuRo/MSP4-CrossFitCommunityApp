...
One noteworthy issue that did turn up was that div elements were nested inside an ol element (in the activity and ranking module ). Apparently this is not correct html, however it did not present any functionality issues. I did manage to change the "ol" element to a "div" element, adding an extra class to identify it and making some minor changes to the relevant js files.
#### User Stories
For the testing of the user stories I refer you to [User Stories](#user-stories). (Click to expand each story instructions.)
#### Programmatic Testing
Because all of the sites major pages are only accessible by authenticated users, and all pages operate using an intricate web of models connected to the get or post request, writing django tests for the views, functions, forms and models seemed overly complex in this context and unneccesarily time consuming. Testing manually in this case was the most optimal choice. As an educational excercise however, I did write a few tests, which you will find under community/test_forms.py, community/test_views.py, home/test_views.py, profiles/models.py. These tests are based on the instruction video's of Code Institute.

Unfortunately I only learned about testing integration software like [Travis](https://travis-ci.org/) around the time I finished developing the app. It was not used during this project but is definitely worth looking at for a next project.
